...
namespace PrinterOffice {
    ...
    /// </summary>
    public partial class DeviceTypeControl : Window {

        private readonly bool isEditMode;
        private readonly DeviceType entity;
        ...
        public DeviceTypeControl(int id) {
            InitializeComponent();
            isEditMode = true;

            Header.Content = "Изменение существующего типа";
            AcceptClientBtn.Content = "Применить";
            DeleteBtn.Visibility = Visibility.Visible;

            var dt = new DataTable();
            Utilities.fillDataTable(dt, "SELECT * FROM deviceTypes WHERE id = @ID", new MySqlParameter("@ID", id));

            TitleBox.Text = dt.Rows[0].Field<string>("title");

            entity = new DeviceType(id);
        }
